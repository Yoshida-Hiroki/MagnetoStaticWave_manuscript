...
%DIF DEL 260222_MSEW_v2.tex   Fri Apr  3 19:42:30 2026
%DIF ADD 260222_MSEW.tex      Fri Apr  3 20:15:30 2026
...
\begin{abstract}
    Magnetoelastic coupling mediated by magnetic dipolar interaction is theoretically investigated in a ferromagnetic thin film under an in-plane magnetic field. We develop a theoretical description that incorporates dipolar fields derived from Maxwell's equations in the presence of elastic deformations. The resulting coupled equations of motion predict hybridization between magnetostatic and Lamb waves. Numerical calculations for a yttrium iron garnet (YIG) film reveal anti-crossings in the dispersion relations, with hybridization gaps ranging from $0.1$ to several MHz.
\end{abstract}
...
In this Letter, we \DIFdelbegin \DIFdel{extend the }\DIFdelend \DIFaddbegin \DIFadd{present a formulation of magnetoelastic coupling between magnetostatic and elastic waves mediated by dipolar interactions. Using a }\DIFaddend Green-function \DIFdelbegin \DIFdel{method }\DIFdelend \DIFaddbegin \DIFadd{approach along the same lines as that }\DIFaddend developed by Kalinikos and Slavin~\cite{Kalinikos_Slavin_1986}\DIFdelbegin \DIFdel{to incorporate dipolar interactions in the presence of elastic deformations . This approach allows us to quantitatively evaluate the magnetoelastic couplingmediated by dipolar interactions}\DIFdelend \DIFaddbegin \DIFadd{, we consistently incorporate elastic deformations into the description, thereby enabling a quantitative evaluation of the coupling}\DIFaddend .
...
\begin{figure}
    \begin{center}
        ...
        \caption{Schematic illustration of the system under consideration. A ferromagnetic thin film with thickness $L$ in the $z$ direction and infinite extent in the $x$ and $y$ directions is considered. Magnetic moments precess around an axis parallel to the in-plane external magnetic field $\bm{H}_0$\DIFaddbeginFL \DIFaddFL{, which is applied at an angle $\varphi$ with respect to the $x$ axis}\DIFaddendFL . The elastic wave changes the positions of the magnetic moments and their separations, resulting in a modification of the dipolar interaction among them.}
        \label{fig:Schematic}
    \end{center}
\end{figure}

We decompose the magnetic field $\bm{H}$ and the magnetization $\bm{M}$ as $\bm{H}=\bm{H}_0+\bm{h}(\bm{r},t)$ and $\bm{M}=\bm{M}_0+\bm{m}(\bm{r},t)$, respectively, where $\bm{r}=(x,y,z)$ is the Euler coordinate, and study propagating magnetostatic waves of small deviations from the equilibrium magnetization. The system also supports elastic waves described by the displacement $\bm{u}(\bm{r},t)$. The deformation modulates the distance between magnetic dipoles, so that the dipolar field $\bm{h}(\bm{r},t)$ generated by $\bm{m}$ is influenced by $\bm{u}$, leading to a magnetoelastic wave. We take the $x$ axis \DIFaddbegin \DIFadd{to be }\DIFaddend parallel to the propagation direction \DIFdelbegin \DIFdel{, and assume that waves are plane wavesand the system is }\DIFdelend \DIFaddbegin \DIFadd{and consider plane waves. The system is assumed to be }\DIFaddend uniform in the $y$ direction. Then, $\bm{m}$ and $\bm{u}$ can be written as
\begin{equation}
...
\end{equation}
where in the second equation the magnetostatic approximation is applied and the time derivative of the electric field is discarded. This approximation is justified \DIFdelbegin \DIFdel{when the steady current is absent and the characteristic frequency }\DIFdelend \DIFaddbegin \DIFadd{in the absence of steady currents and under the quasi-static condition $\omega L / c \ll 1$, where }\DIFaddend $\omega$ and \DIFaddbegin \DIFadd{$L$ are the characteristic frequency and }\DIFaddend length scale \DIFdelbegin \DIFdel{$L$ }\DIFdelend of the magnetization dynamics \DIFdelbegin \DIFdel{satisfy the quasi-static condition $\omega L/c \ll 1$, so that }\DIFdelend \DIFaddbegin \DIFadd{and $c$ is the speed of light. In this regime, }\DIFaddend retardation effects and electromagnetic radiation can be \DIFdelbegin \DIFdel{ignored}\DIFdelend \DIFaddbegin \DIFadd{neglected}\DIFaddend ~\cite{Stancil_TheoryofMagnetostaticWaves}. By the plane-wave assumption~(\ref{eq:Assumption}), the dipolar field can also be written as $\bm{h}=\bm{h}(z)e^{i(kx-\omega t)}$. We rewrite Eq.~(\ref{eq:Maxwell_R}) in terms of these quantities in the Euler coordinate $\bm{r}$. We note that we need to pay attention to the conservation of the magnetization in a unit volume~\cite{Yamamoto_Maekawa_AnnPhys} yielding $\bm{M}(\bm{R})=\bm{M}(\bm{r})/\det(J(\bm{r}))$, where $J(\bm{r})$ is a Jacobian matrix (see Supplementary data~\cite{SM} for details). We get
\begin{equation}
...
\end{equation}
This dipolar field couples \DIFdelbegin \DIFdel{the magnetostatic wave and the elastic wave }\DIFdelend \DIFaddbegin \DIFadd{magnetostatic and elastic waves }\DIFaddend in the film. \DIFdelbegin \DIFdel{We note that in }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend the present setup\DIFdelbegin \DIFdel{there is no coupling }\DIFdelend \DIFaddbegin \DIFadd{, only elastic deformations in the $xz$ plane couple to the magnetization. In addition, the coupling vanishes }\DIFaddend when the equilibrium magnetization is parallel to the $y$ axis.
...
\end{equation}
where the asterisk \DIFdelbegin \DIFdel{shows }\DIFdelend \DIFaddbegin \DIFadd{denotes }\DIFaddend complex conjugate. We can verify this by constructing the Euler--Lagrange equation with respect to $\bm{m}^*$ from this Lagrangian. On the other hand, the dynamics of the displacement field $\bm{u}$ is described by the Navier--Cauchy equation
\begin{equation}
...
In order to examine the dynamical properties of the coupled waves described by the above equations of motion, we numerically calculate the dispersion relation in the following. Under the assumption~(\ref{eq:Assumption}), the time derivatives on the left-hand side of the equations become $-i\omega$. \DIFdelbegin \DIFdel{The second of Eqs.~(\ref{eq:CoupledEOM}) contains spatial derivatives of $\bm{u}$ and requires boundary conditions to be solved. }\DIFdelend Since we consider a film in vacuum, we impose the stress-free condition at the surfaces $z=\pm L/2$ as
\begin{equation}
    \sigma_{xz}|_{z=\pm\frac{L}{2}}=\sigma_{yz}|_{z=\pm\frac{L}{2}}=\sigma_{zz}|_{z=\pm\frac{L}{2}}=0.\label{eq:BC_EW}
\end{equation}
Without the coupling, it is known that a type of elastic wave called a Lamb wave appears under these conditions. Since the system in equilibrium is symmetric with respect to the $z=0$ plane, the Lamb wave modes are either even or odd in $z$. \DIFaddbegin \DIFadd{For later convenience, we introduce the $C_{2x}$ symmetry operation. The system is invariant under $C_{2x}$, and the deformation vector $\bm{u}(z)$ is an eigenfunction of the corresponding operator: $C_{2x}\bm{u}(z)=\pm\bm{u}(z)$. The eigenvalues $\pm 1$ correspond to mirror-symmetric and antisymmetric Lamb modes, respectively. }\DIFaddend Numerically, we need to solve the equations under the boundary conditions~(\ref{eq:BC_EW}). For example, the finite element method naturally incorporates these conditions.
...
For the calculation of the dispersion, it is convenient to introduce another coordinate system, where the original one $(x,y,z)$ is rotated around the $z$ axis so that one of the \DIFdelbegin \DIFdel{axis }\DIFdelend \DIFaddbegin \DIFadd{axes }\DIFaddend is parallel to the external magnetic field. Then, the component of $\bm{m}$ is zero along this direction since $\bm{M}_0\cdot\bm{m}=0$ from the constraint $|\bm{M}|=const.$
