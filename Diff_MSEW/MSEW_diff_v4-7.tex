...
%DIF DEL 260222_MSEW_v4.tex   Mon Apr 13 19:24:01 2026
%DIF ADD 260222_MSEW_v7.tex   Sat May 23 16:35:30 2026
%DIF < % journal:APEX
%DIF < % author:Hiroki Yoshida
%DIF -------
...
\DIFdelbegin %DIFDELCMD < \submitto{\APEX}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \submitto{\JJAP}
\DIFaddend \maketitle
\ioptwocol
...
\DIFadd{arises from the $\bm{h}^*\cdot\bm{h}$ term in the Lagrangian. This term is analogous to the standard local magnetoelastic coupling density~\mbox{%DIFAUXCMD
\cite{Kittel1949,KittelAbrahams1953,Neel1954, CallenCallen1963}}\hskip0pt%DIFAUXCMD
, while explicitly retaining the long-range character of the dipolar interaction. In addition, the elasticity tensor receives a magnetostatic contribution, and }\DIFaddend a new volume force term $-\bm{f}$ appears in the Navier--Cauchy equation~(\ref{eq:NavierCauchy}), where the force density $\bm{f}$ is given by (see Supplementary data~\cite{SM} for details of the derivation)
\begin{equation}
