...
%DIF DEL 260222_MSEW_v3.tex   Fri Apr  3 20:15:30 2026
%DIF ADD 260222_MSEW_v4.tex   Mon Apr 13 19:24:01 2026
...
\begin{document}
\title{Magnetoelastic \DIFdelbegin \DIFdel{wave }\DIFdelend \DIFaddbegin \DIFadd{Waves }\DIFaddend in Ferromagnetic \DIFdelbegin \DIFdel{Thin-film }\DIFdelend \DIFaddbegin \DIFadd{Thin Films }\DIFaddend Mediated by Dipolar \DIFdelbegin \DIFdel{Interaction}\DIFdelend \DIFaddbegin \DIFadd{Interactions}\DIFaddend }
%DIF <  \article{\APEX}{Magnetoelastic wave in Ferromagnetic Thin-film Mediated by Dipolar Interaction}
...
\begin{abstract}
    Magnetoelastic coupling mediated by magnetic dipolar \DIFdelbegin \DIFdel{interaction }\DIFdelend \DIFaddbegin \DIFadd{interactions }\DIFaddend is theoretically investigated in \DIFdelbegin \DIFdel{a ferromagnetic thin film }\DIFdelend \DIFaddbegin \DIFadd{ferromagnetic thin films }\DIFaddend under an in-plane magnetic field. We develop a theoretical description that incorporates dipolar fields derived from Maxwell's equations in the presence of elastic deformations. The resulting coupled equations of motion predict hybridization between magnetostatic and Lamb waves. Numerical calculations for a yttrium iron garnet (YIG) film reveal anti-crossings in the dispersion relations, with hybridization gaps ranging from $0.1$ to several MHz.
\end{abstract}

%DIF <  \keywords{Universe}
\submitto{\APEX}
\maketitle
\ioptwocol
...
\end{equation}
Without the coupling, it is known that a type of elastic wave called a Lamb wave appears under these conditions. Since the system in equilibrium is symmetric with respect to the $z=0$ plane, the Lamb wave modes are either even or odd in $z$. For later convenience, we introduce the $C_{2x}$ symmetry operation. The system is invariant under $C_{2x}$, and the deformation vector $\bm{u}(z)$ is an eigenfunction of the corresponding operator: $C_{2x}\bm{u}(z)=\pm\bm{u}(z)$. \DIFdelbegin \DIFdel{The eigenvalues $\pm 1$ correspond to mirror-symmetric and antisymmetric Lamb modes, respectively. }\DIFdelend Numerically, we need to solve the equations under the boundary conditions~(\ref{eq:BC_EW}). For example, the finite element method naturally incorporates these conditions.
...
\DIFdelbegin \DIFdel{From the same hybridization gaps, we further estimate }\DIFdelend \DIFaddbegin \DIFadd{To generate the same gap via magnetoelastic coupling for bulk transverse waves, }\DIFaddend the magnetoelastic coupling constant $b_2$ \DIFaddbegin \DIFadd{would need }\DIFaddend to be on the order of $10^4$ J/m$^3$~\cite{Hioki2022}, which is one order of magnitude smaller than the experimental value \DIFaddbegin \DIFadd{for YIG}\DIFaddend , $b_2 \sim 10^5$ J/m$^3$~\cite{Krysztofik2021}. This result indicates that the dipolar contribution to the magnetoelastic coupling is subdominant compared to other mechanisms in realistic systems.
...
\DIFdelbegin %DIFDELCMD < \ack{H. Y. acknowledges support from Japan Society for the Promotion of Science (JSPS) KAKENHI Grant No. JP24KJ1109 and by MEXT Initiative to Establish Next-generation Novel Integrated Circuits Centers (X-NICS) Grant No. JPJ011438. S. M. acknowledges support by Japan Society for the Promotion of Science (JSPS) KAKENHI Grant No. JP22K18687, No. JP22H00108, and No. JP24H02231.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ack{H. Y. acknowledges support from Japan Society for the Promotion of Science (JSPS) KAKENHI Grant Number JP24KJ1109 and MEXT Initiative to Establish Next-generation Novel Integrated Circuits Centers (X-NICS) Grant Number JPJ011438. 
