...
\begin{equation}
    ...
    \right]
\end{equation}
arises from the $\bm{h}^*\cdot\bm{h}$ term in the Lagrangian. This term is analogous to the standard local magnetoelastic coupling density~\cite{Kittel1949,KittelAbrahams1953,CallenCallen1963}, while retaining the explicitly long-range character of the dipolar interaction. From this coupling, a new volume force term $-\bm{f}$ appears in the Navier--Cauchy equation~(\ref{eq:NavierCauchy}), where the force density $\bm{f}$ is given by (see Supplementary data~\cite{SM} for details of the derivation)
\begin{equation}
